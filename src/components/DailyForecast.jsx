  ...
  if (data) {
    ...
    for (let i = 0; i < data.list.length; i++) {
      ...
      if (
        ...
        today != new Date(dt_txt).toLocaleDateString()
      ) {
        currentTimeStamp = dt_txt;
        ...
        {dailyWeatherData.length ? (
          dailyWeatherData.map((item, index) => {
            return (
              <div
                key={index}
                className="flex items-center xl:justify-between xl:w-[370px] "
              >
                <img
                  className=" lg:w-8 xl:w-[60px]"
                  src={`https://openweathermap.org/img/wn/${
                    item && item.weather[0].icon
                  }@2x.png`}
                  alt=""
                />
                <p className="xl:text-2xl font-semibold">
                  {parseInt(item.main.temp)}°C
                </p>
                <div className="w-[163px] font-semibold">
