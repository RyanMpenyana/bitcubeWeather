...
function App() {
  ...
  const getCurrentWeather = async (location) => {
    ...
    try {
      let response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${location? location : search}&appid=${API_KEY}&units=metric`)
  ...
  const getForecastWeatherData = async (location) => {
    try {
      let response = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${location? location : search}&appid=${API_KEY}&units=metric`)
      let data = await response.json()
  ...
  return (
    ...
    <AppContext.Provider value={data}>
      ...
      <ForecastContext.Provider value={forecastData}>
        <div className={`App text-center gap-4 ${isDarkMode ? 'text-black' : 'text-white'}  pb-16 h-full  px-[20px] xl:px-5`}>
